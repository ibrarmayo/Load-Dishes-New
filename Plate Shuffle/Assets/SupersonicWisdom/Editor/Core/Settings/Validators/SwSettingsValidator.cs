...
namespace SupersonicWisdomSDK.Editor
{
    internal abstract class SwSettingsValidator
    {
        ...
        {
            if (param == MissingParam.FbClientToken)
            {
                if (SwEditorAlerts.AlertError(SwEditorConstants.UI.PARAM_IS_MISSING.Format(GetDetails(param)), (long)SwErrors.EBuild.MissingFacebookTokenSettings, SwEditorConstants.UI.ButtonTitle.GO_TO_SETTINGS, SwEditorConstants.UI.ButtonTitle.CLOSE))
                {
                    SwMenu.AllowEditingSettings();
                    SwEditorUtils.OpenSettings();
                }
                
                throw new BuildPlayerWindow.BuildMethodException("Facebook token is missing.");
            }
            else
            {
