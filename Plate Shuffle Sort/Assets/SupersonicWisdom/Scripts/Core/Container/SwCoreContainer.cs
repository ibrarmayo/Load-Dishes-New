    ...
    internal abstract class SwCoreContainer : ISwContainer
    {
        ...
        protected IEnumerator NotifyReadiness()
        {
            ...
            try
            {
                ...
                SwInfra.Logger.Log(EWisdomLogType.Container, "SwCoreContainer | OnReadyEvent");
                OnReadyEvent?.Invoke();
            }
            catch (Exception e)
            {
                SwInfra.Logger.LogError(EWisdomLogType.Container, $"SwCoreContainer | OnReadyEvent | Error | {e.Message}");
