...
namespace SupersonicWisdomSDK
{
    internal class SwDeepLinkHandler
    {
        ...
        private IEnumerator VerifyDeepLinkParams(Dictionary<string, string> deepLinkParams)
        {
            ...
            if (isVerified)
            {
                SwInfra.Logger.Log(EWisdomLogType.DeepLink, $"Verified deep link params | {SwJsonParser.Serialize(deepLinkParams)}");
            }
            else
            {
                SwInfra.Logger.LogError(EWisdomLogType.DeepLink, $"Deep link params verification failed | {SwJsonParser.Serialize(deepLinkParams)}");
